...
def memory_sampling_balanced(opts, model_prev):
    ...
    train_loader = data.DataLoader(
        train_dst, batch_size=opts.dataset.batch_size,
        shuffle=False, num_workers=4, drop_last=False)
    
    num_classes = [len(get_tasks(opts.dataset.name, opts.task, step)) for step in range(opts.curr_step+1)]
    prev_num_classes = sum(num_classes[:-1])  # 16
    memory_json = f'./datasets/data/{opts.dataset.name}/memory.json'
    ...
    for images, targets, img_names in train_loader:
        ...
        with torch.no_grad():
            ...
            if opts.curr_step > 1:
                ...
                outputs, _, _, _ = model_prev(images)
                
                if opts.train.loss_type == 'ce_loss':
