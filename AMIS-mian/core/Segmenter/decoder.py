...
class MaskTransformer(nn.Module):
    def __init__(
        ...
        self.general_emb = nn.ParameterList([nn.Parameter(torch.randn(1, 1, d_model))])
        ...
        self.mlp = FeedForward(dim=768, hidden_dim=1536, dropout=0.1)
        self.patches = None
    ...
    def forward(self, x, im_size):
        ...
        general_embeds = [general_embed for general_embed in self.general_emb]
        general_embeds = torch.cat(general_embeds, dim=1)
        general_embeds = general_embeds.expand(x.size(0), -1, -1)
        self.cls_token = cls_embeds
        x = torch.cat((x, cls_embeds), 1)
        
        for blk in self.blocks:
            layer_number += 1
            if layer_number >= 1 and general_embeds is not None:
                general_token = general_embeds.expand(B, 1, -1)
